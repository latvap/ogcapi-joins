...
The standard contains two  data joining methods:

* The first data joining method consists of joining data with a collection available on the server. In this method, the data is first saved to the server as a join source. The join source data can be then joined with a collection by executing a /items path query and providing the join identifier as a parameter to that query. This method allows the collection data to be filtered dynamically before the data is joined with the resulting collection items data.
* The second data joining method executes on-the-fly data joining between two input files.
...
* Creating a join source for a specific collection by saving data on the server.
* Getting metadata of the join sources on the server.
* Joining data from a join source with a collection available on the server.
* Deleting joins sources from the server.
* Executing on-the-fly joins between two input files.

The input data files can be either uploaded to the server with the join source creation or the direct join query or provided by referencing them via URL links. The server implementations have a mandatory requirement to support the GeoJSON format for the joined data, and in addition, they can support any other file formats. 
...
If several of these requests are processed simultaneously, the server implementations can become slow or unresponsive. Servers should set and apply reasonable limits to the sizes of the input data files to prevent Denial of Service attacks.
