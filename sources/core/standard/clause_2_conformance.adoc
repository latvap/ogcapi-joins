...
The __data joining operations__ operation set is represented by the requirements classes: "<<rc_data_joining,data joining>>" and "<<rc_join_source_delete,join source delete>>". The implementation of the "data joining" requirements class is mandatory, and the implementation of the "join source delete" is optional for this operation set.

The __file joining operations__ operation set is represented by the requirements class "<<rc_file_joining,file joining>>". 

The input and output functionalities are represented by the requirement classes: "<<rc_file_uploading_with_query, Input File Upload>>", "<<rc_file_referencing_with_url, File Referencing with URL>>", "<<rc_input_csv, CSV File Input>>" and "<<rc_input_geojson,GeoJSON File Input>>". The support for CSV input file format is mandatory for the __data joining operations__ and __file joining operations__. The support for the GeoJSON input file format is mandatory for the __file joining operations__.
...
<|<<ats_join_source_delete,Join Source Delete>>|http://www.opengis.net/spec/ogcapi-joins-1/1.0/conf/join-source-delete
