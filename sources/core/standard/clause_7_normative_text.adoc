...
The HTTP GET operation at path `{root}/collections/{collectionId}/joins` returns a list of join sources  available on the server that have been created for the specific collection.

==== Request

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-op.adoc[]

include::./requirements/data-joining/PER_collections-collectionid-joins-get-op-limit-parameter.adoc[]

include::./requirements/data-joining/PER_collections-collectionid-joins-get-op-datetime-parameter.adoc[]

==== Response

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success.adoc[]

===== Parameter Limit

The number of returned join sources depends on the server and the value of the `limit` parameter. 

The client can request a limit to the number of join sources returned in a response by using the `limit` parameter. The `limit` parameter indicates the maximum number of join sources which should be included in a single response.

The server may have a default value for the limit and a maximum limit.

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-limit.adoc[]

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-limit-unsupported.adoc[]

===== Parameter datetime

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-datetime.adoc[]
...
include::./requirements/data-joining/PER_collections-collectionid-joins-get-success-server-limit.adoc[]

include::./requirements/data-joining/REC_collections-collectionid-joins-get-success-server-limit.adoc[]
...
include::./requirements/data-joining/REC_collections-collectionid-joins-get-success-next-1.adoc[]

include::./requirements/data-joining/REC_collections-collectionid-joins-get-success-next-2.adoc[]

include::./requirements/data-joining/REC_collections-collectionid-joins-get-success-next-3.adoc[]
...
include::./requirements/data-joining/PER_collections-collectionid-joins-get-success-prev.adoc[]

If the server response does not contain all of the join sources that match the selection parameters, then the client must be informed of that fact.

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-paged-response.adoc[]

The `numberMatched` property of the response indicates the number of join sources that are available in the server that match the selection parameters in the request.

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-numberMatched.adoc[]

include::./requirements/data-joining/PER_collections-collectionid-joins-get-success-numberMatched.adoc[]

The number of join sources included in a response may be a subset of the number matched. In that case, the `numberReturned` property of the response indicates the number of join sources returned in this "page" of the response.

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-numberReturned.adoc[]

include::./requirements/data-joining/PER_collections-collectionid-joins-get-success-numberReturned.adoc[]

===== Response schema for the Join Sources
include::../schemas/collections_collectionid_joins_schema.adoc[]


[[join-sources-resource-definition]]
//[discrete]
====== joins
The `joins` property of the response provides a description of each individual join source hosted by the server.

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-items.adoc[]

//[discrete]
====== links

To support hypermedia navigation, the `links` property must be populated with sufficient hyperlinks to navigate through the join sources.

Property `links` in the `CollectionJoinsResponseObject`:

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-links.adoc[]

Property `links` in the `JoinsObject`:

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-items-links.adoc[]

//[discrete]
====== id

Identifier of the join source.

//[discrete]
====== joins

An array containing a list of join sources.

//[discrete]
====== numberMatched

The number of join sources that are available in the server that match the selection parameters in the request.

//[discrete]
====== numberReturned

The number of join sources returned in the response.

//[discrete]
====== timeStamp

Property `timeStamp` in the `CollectionJoinsResponseObject`:

include::./requirements/data-joining/REQ_collections-collectionid-joins-get-success-timeStamp.adoc[]

Property `timeStamp` in the `JoinsObject`:

The time when the join source was generated.

==== Error Situations
See <<http_status_codes,HTTP status codes>> for general guidance.

[[join_source_creation]]
=== Join Source Creation

The HTTP POST operation at path `{root}/collections/{collectionId}/joins` creates a new join source. 

The operation reads data from an input data file and saves it to the server as a join source.
...
The core module contains support for the CSV format for the input data files, defined in the requirements class "<<rc_input_csv, CSV File Input>>". The CSV files must have a column that contains the key values and a separate column for each joined attribute.

The joins are executed when the collection data items query is requested with the joinId parameter by using common key values that are available both, in the join source data and in the collection data.

==== Request

include::./requirements/data-joining/REQ_collections-collectionid-joins-post-op.adoc[]

include::./requirements/data-joining/REQ_collections-collectionid-joins-post-op-form-data-parameters.adoc[]

==== Response

include::./requirements/data-joining/REQ_collections-collectionid-joins-post-success.adoc[]

===== Response schema for the Join Source Creation

The response schema for queries that create a join resource is expressed with the <<collections_collectionid_join_joinid_schema,Join Schema>>.
...
The HTTP GET operation at path `{root}/collections/{collectionId}/joins/{joinId}` returns metadata on a specific join source that is available on the server.

==== Request

include::./requirements/data-joining/REQ_collections-collectionid-joins-joinid-get-op.adoc[]

==== Response

include::./requirements/data-joining/REQ_collections-collectionid-joins-joinid-get-success.adoc[]

===== Response Schema for the Join Source

include::./../schemas/collections_collectionid_joins_join_schema.adoc[]

[[req_core_collections-collectionid-joins-joinid-resource-definition]]

//[discrete]
====== details

Details of the join source.

//[discrete]
====== collectionKey

Key of the collection that matches to the joinKey.

//[discrete]
====== created

The time of the join source creation.
...
Names of the data fields in the join source.

//[discrete]
====== joinKey

Name of the property that is the join source key.

//[discrete]
====== numberOfRows

The number of rows in the join source.
...
Unique identifier for the join source.
...
include::./requirements/data-joining/REQ_collections-collectionid-joins-joinid-get-success-links.adoc[]

//[discrete]
====== timeStamp

The time when the join souce output was requested.
...
[[rc_join_source_delete]]
== Requirements Class "Join Source Delete"

=== Overview
include::./requirements/requirements_class_join_source_delete.adoc[]

=== Join Source Delete

The HTTP DELETE operation at path `{root}/collections/{collectionId}/joins/{joinId}` deletes the specific join source from the server. 

==== Request

include::./requirements/join-source-delete/REQ_collections-collectionid-joins-joinid-delete-op.adoc[]

==== Response

include::./requirements/join-source-delete/REQ_collections-collectionid-joins-joinid-delete-success.adoc[]

===== Response schema for the Join Source Delete
...
The <<join_source_creation,Join Source Creation>> operation of the Data Joining requirement class defines how to save a join source data to a server. The Input File Upload requirements class specifies mechanisms for specifying support for input file uploading to the server.

==== Form Data Parameter `right-dataset-file`
include::./requirements/input-file-upload/REQ_right-dataset-file-join-source-creation.adoc[]
...
=== Join Creation Operation
The <<join_source_creation,Join Source Creation>> operation of the Data Joining requirement class defines how to save join source data to a server. The File Referencing with URL requirements class specifies mechanisms for specifying support for referencing input files via URL links.

==== Form Data Parameter `right-dataset-url`
include::./requirements/input-http-ref/REQ_right-dataset-url-join-source-creation.adoc[]
...
=== Join Creation Operation
The <<join_source_creation,Join Source Creation>> operation of the Data Joining requirement class defines how to save a Join Source data to a server. The CSV File Input requirements class specifies mechanisms for specifying support for CSV input data format.

==== Parameter `right-dataset-format`
include::./requirements/input-csv/REQ_right-dataset-format-join-source-creation.adoc[]
