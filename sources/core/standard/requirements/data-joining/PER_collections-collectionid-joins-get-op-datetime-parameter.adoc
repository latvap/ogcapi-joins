[[per_data_joining_collections-collectionid-joins-get-op-datetime-parameter]]

[permission]
====
[%metadata]
identifier:: /per/data-joining/collections-collectionid-joins-get-op-datetime-parameter
part:: 
The server MAY support the query parameter `datetime` to filter the returned join sources by their execution timestamp.
